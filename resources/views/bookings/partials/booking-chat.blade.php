        ...
        <div class="px-3 py-3 sm:px-5 sm:py-4">
                ...
                <template x-for="m in messages" :key="m.id">
                    <div class="flex px-1" :class="m.is_me ? 'justify-end' : 'justify-start'">
                        ...
                        >
                            ...
                            <p class="mt-0.5 whitespace-pre-wrap break-words" x-show="m.body && m.body.trim()" x-text="m.body"></p>
            ...
            <div class="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                <input
                    ...
                    class="block w-full text-xs text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-slate-800 hover:file:bg-slate-200 disabled:opacity-50"
                    :disabled="!chatOpen || sending"
                    @change="pickImage($event)"
                >
                <p class="text-[11px] text-slate-500 sm:flex-1">{{ __('bookings.chat.image_hint') }}</p>
                <button
                    type="button"
            ...
            <div x-show="imagePreviewUrl" x-cloak class="mt-2 rounded-xl border border-slate-200 bg-white p-2">
                <img :src="imagePreviewUrl" alt="" class="max-h-40 max-w-full rounded-lg object-contain">
            </div>
