@php
    ...
    $opportunityAddonsById = $opportunityAddonsById ?? collect();
    $hasPendingItems = $tab !== 'pending' || $replacements->isNotEmpty() || $openIncidents->isNotEmpty();
    $emptyKey = match ($tab) {
        'active' => 'muthowif.replacements.empty_active',
        'history' => 'muthowif.replacements.empty_history',
        default => 'muthowif.replacements.empty_pending',
    };
@endphp
    ...
    >
        <x-page-container class="space-y-6 py-2 sm:py-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <a href="{{ route('muthowif.bookings.index') }}" class="inline-flex shrink-0 items-center gap-2 self-start rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition hover:border-slate-300 hover:bg-slate-50">
            ...
            @else
                    ...
                    @if ($tab === 'pending')
                        @foreach ($replacements as $replacement)
                            @include('muthowif.bookings.partials.replacement-invite-card', [
                                ...
                                'defaultOpen' => $loop->first && $openIncidents->isEmpty(),
                            ])
                        @endforeach
                        @foreach ($openIncidents as $incident)
                            ...
                            @if ($booking)
                                @include('muthowif.bookings.partials.replacement-request-card', [
                                    ...
                                    'booking' => $booking,
                                    'incident' => $incident,
                                    'addonsById' => $opportunityAddonsById,
                                    'defaultOpen' => $loop->first && $replacements->isEmpty(),
                                    'interactive' => true,
                                ])
                            @endif
                        @endforeach
                ...
                @if ($tab !== 'pending' && $replacements->hasPages())
