...
<x-page-container class="ui-stack-compact py-2 sm:py-4">
    ...
    @if ($bookings->isEmpty())
        ...
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm sm:py-14">
            ...
            @if ($filteredStatus)
                <p class="mt-4 text-base font-semibold text-slate-900">{{ __('muthowif.bookings.empty_filtered_title', ['status' => $filteredStatus->label()]) }}</p>
                <p class="mx-auto mt-2 max-w-md text-sm text-slate-600">{{ __('muthowif.bookings.empty_filtered_lead') }}</p>
                <a href="{{ route('muthowif.bookings.index') }}" class="mt-6 inline-flex items-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-800 shadow-sm hover:bg-slate-50">
                    {{ __('muthowif.bookings.filter_all') }}
                </a>
            @else
