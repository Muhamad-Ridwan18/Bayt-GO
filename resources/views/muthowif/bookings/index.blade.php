                            ...
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                <div class="min-w-0">
                                    ...
                                    @elseif ($st === BookingStatus::Completed)
                                        @if ($booking->payment_status === PaymentStatus::Paid)
                                            <p class="mt-1 text-sm font-semibold text-emerald-800">{{ __('muthowif.bookings.completed_notice') }}</p>
                                        @endif
                                    @endif
                                </div>
                                <div class="flex flex-wrap gap-2 shrink-0">
                                    @if ($st === BookingStatus::Pending)
                                        ...
                                        <form method="POST" action="{{ route('muthowif.bookings.cancel', $booking) }}" class="inline" onsubmit="return confirm(@json(__('muthowif.bookings.reject_confirm')));">
                                            @csrf
                                    ...
                                    @elseif ($st === BookingStatus::Confirmed && $booking->payment_status === PaymentStatus::Pending)
                                        <form method="POST" action="{{ route('muthowif.bookings.cancel', $booking) }}" class="inline" onsubmit="return confirm(@json(__('muthowif.bookings.cancel_unpaid_confirm')));">
                                            @csrf
