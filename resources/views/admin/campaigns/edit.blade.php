        ...
        <x-page-container class="ui-stack relative">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">{{ $campaign->title }}</p>
                </div>
            </div>

            <form action="{{ route('admin.campaign.update', $campaign) }}" method="POST" enctype="multipart/form-data" class="ui-stack-compact">
                @csrf
                @method('PUT')
                    ...
                    <div class="ui-card-pad-lg ui-stack-compact">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <x-text-input id="start_date" name="start_date" type="datetime-local" class="mt-1 block w-full" :value="old('start_date', $campaign->start_date?->format('Y-m-d\TH:i'))" required />
                                <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="end_date" value="Tanggal Selesai *" />
                                <x-text-input id="end_date" name="end_date" type="datetime-local" class="mt-1 block w-full" :value="old('end_date', $campaign->end_date?->format('Y-m-d\TH:i'))" required />
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                @if($campaign->desktop_banner)
                                    <div class="mb-2"><img src="{{ Storage::url($campaign->desktop_banner) }}" class="h-20 object-cover rounded" /></div>
                                @endif
                            ...
                            <div>
                                ...
                                @if($campaign->mobile_banner)
                                    <div class="mb-2"><img src="{{ Storage::url($campaign->mobile_banner) }}" class="h-20 object-cover rounded" /></div>
                                @endif
                        ...
                        <div>
                            ...
                            <textarea id="body" name="body" rows="4" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">{{ old('body', $campaign->body) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                            <div>
                                ...
                                <x-text-input id="cta_text" name="cta_text" type="text" class="mt-1 block w-full" :value="old('cta_text', $campaign->cta_text)" />
                            </div>
                            <div>
                                <x-input-label for="cta_url" value="URL Tombol CTA" />
                                <x-text-input id="cta_url" name="cta_url" type="text" class="mt-1 block w-full" :value="old('cta_url', $campaign->cta_url)" />
                            </div>
                            <div>
                                <x-input-label for="theme_color" value="Warna Tema (Hex)" />
                                <x-text-input id="theme_color" name="theme_color" type="text" class="mt-1 block w-full" :value="old('theme_color', $campaign->theme_color)" />
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-6">
                            <div>
                                ...
                                <x-text-input id="sort_order" name="sort_order" type="number" class="mt-1 block w-full" :value="old('sort_order', $campaign->sort_order)" />
                            </div>
                            <div class="mt-6 flex items-center">
                                <input id="is_active" name="is_active" type="checkbox" value="1" class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-600" {{ old('is_active', $campaign->is_active) ? 'checked' : '' }}>
