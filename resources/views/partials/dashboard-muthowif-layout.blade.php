    ...
    <div class="contents lg:col-span-8 lg:flex lg:flex-col lg:gap-6">
        ...
        <section class="relative order-1 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-md ring-1 ring-slate-100/80 lg:order-none">
            <div class="pointer-events-none absolute inset-0 z-0" aria-hidden="true">
                <img src="{{ $page->heroBgUrl }}" alt="" class="h-full w-full min-h-[11rem] object-cover object-[80%_35%] sm:min-h-[13rem]" loading="eager" decoding="async" />
            </div>
            ...
            <div class="relative z-10 flex flex-col gap-4 p-6 sm:p-8 lg:flex-row lg:items-stretch lg:gap-6">
                <div class="min-w-0 flex-1 lg:max-w-md">
                    <p class="text-xl font-bold leading-snug text-slate-900 sm:text-2xl">
                        {!! __('dashboard_muthowif.hero_hi_html', ['name' => e($page->userName)]) !!}
                    </p>
                    <p class="mt-2 text-sm text-slate-600">{{ __('dashboard_muthowif.hero_sub') }}</p>
                    @if ($page->activeBookingsCount > 0)
                        <p class="mt-1 text-sm font-medium text-baytgo">{{ __('dashboard_muthowif.hero_sub_active', ['count' => $page->activeBookingsCount]) }}</p>
                    @endif
                ...
                @if ($page->nextBooking)
                    <div class="lg:flex lg:w-72 lg:shrink-0 lg:items-end">
                        <div class="w-full rounded-2xl border border-slate-200/90 bg-white p-4 shadow-lg ring-1 ring-white/80">
                            ...
                            <p class="mt-1 text-lg font-bold text-slate-900">{{ $page->nextBooking['guest'] }}</p>
                            <p class="mt-0.5 text-sm text-slate-600">{{ $page->nextBooking['date_label'] }}</p>
                            <p class="mt-0.5 text-xs text-slate-500">{{ $page->nextBooking['service_label'] }}</p>
                            <div class="mt-3 flex items-center justify-between gap-2">
                                <span class="rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $page->nextBooking['status_pill'] }}">{{ $page->nextBooking['status_label'] }}</span>
                                <a href="{{ $page->nextBooking['href'] }}" class="text-xs font-semibold text-baytgo hover:text-baytgo-800">{{ __('dashboard_muthowif.view_detail') }} →</a>
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="mt-4 flex gap-3 overflow-x-auto pb-1 sm:gap-4">
                <a href="{{ route('muthowif.bookings.index') }}" class="flex min-w-[4.25rem] flex-col items-center gap-2 text-center">
                    <span class="relative flex h-14 w-14 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-emerald-700 shadow-sm">
                        ...
                        @if ($page->pendingBookingsCount > 0)
                            <span class="absolute -right-0.5 -top-0.5 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white">{{ $page->pendingBookingsCount }}</span>
                        @endif
                    </span>
                ...
                <a href="{{ route('muthowif.emergency-offers.index') }}" class="flex min-w-[4.25rem] flex-col items-center gap-2 text-center">
                    <span class="relative flex h-14 w-14 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-orange-700 shadow-sm">
                        ...
                        @if ($page->pendingEmergencyOffersCount > 0)
                            <span class="absolute -right-0.5 -top-0.5 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white">{{ $page->pendingEmergencyOffersCount > 9 ? '9+' : $page->pendingEmergencyOffersCount }}</span>
                        @endif
                    </span>
                    <span class="text-xs font-semibold text-slate-700">{{ __('nav.emergency_offers') }}</span>
                </a>
                <a href="{{ route('chat.index') }}" class="flex min-w-[4.25rem] flex-col items-center gap-2 text-center">
                    <span class="relative flex h-14 w-14 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-violet-700 shadow-sm">
                        ...
                        @if ($page->unreadChatCount > 0)
                            <span class="absolute -right-0.5 -top-0.5 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white">{{ $page->unreadChatCount > 9 ? '9+' : $page->unreadChatCount }}</span>
                        @endif
                    </span>
                            ...
                            <article class="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-slate-50/50 p-4 sm:flex-row sm:items-center">
                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <p class="text-base font-bold text-slate-900">{{ $row['guest'] }}</p>
                                        <span class="rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $row['status_pill'] }}">{{ $row['status_label'] }}</span>
                                    </div>
                                    <p class="mt-1 text-sm text-slate-600">{{ $row['date_label'] }} · {{ $row['service_label'] }}</p>
                                </div>
                                <div class="flex shrink-0 gap-2">
                                    <a href="{{ $row['href'] }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50">{{ __('dashboard_muthowif.view_detail') }}</a>
                                    <button type="button" @click="$dispatch('open-booking-chat', { bookingId: @js($row['id']) })" class="inline-flex items-center justify-center rounded-xl bg-baytgo px-4 py-2 text-sm font-semibold text-white hover:bg-baytgo-800">{{ __('dashboard_muthowif.chat') }}</button>
                                </div>
                            </article>
                        @endforeach
                    </div>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                @else
                    <ul class="divide-y divide-slate-100">
                        @foreach ($page->weeklySchedule as $row)
                            <li class="flex flex-wrap items-center gap-3 py-3 first:pt-0 last:pb-0">
                                <div class="w-12 shrink-0 text-center">
                                    <p class="text-[10px] font-bold uppercase text-slate-400">{{ $row['month'] }}</p>
                                    <p class="text-lg font-bold text-slate-900">{{ $row['day'] }}</p>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-slate-900">{{ $row['guest'] }}</p>
                                    <p class="text-xs text-slate-500">{{ $row['service_label'] }}</p>
                                </div>
                                <span class="rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $row['status_pill'] }}">{{ $row['status_label'] }}</span>
                            </li>
                        @endforeach
        ...
        <section id="muthowif-schedule" class="order-6 scroll-mt-24 space-y-3 lg:order-none">
            ...
            <div
                ...
                x-data="muthowifDashboardCalendar({
                    ...
                    month: @js($page->calendarData['calendarMonth']->format('Y-m')),
                })"
                @click.capture="onCalendarNavClick($event)"
            >
                <div :class="{ 'opacity-60 pointer-events-none': loading }">
                    @include('partials.muthowif-calendar-panel', $page->calendarData)
                </div>
                <div :class="{ 'opacity-60 pointer-events-none': loading }">
                    @include('partials.muthowif-blocked-panel', $page->calendarData)
                </div>
            </div>
        </section>
    </div>
    ...
    <aside class="contents lg:col-span-4 lg:flex lg:flex-col lg:gap-5">
        <div class="order-2 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm lg:order-none">
            ...
            <a href="{{ route('muthowif.withdrawals.index') }}" class="mt-3 block rounded-2xl bg-gradient-to-br from-baytgo to-baytgo-800 p-5 text-white shadow-md">
                ...
                <p class="mt-1 text-2xl font-bold tabular-nums">Rp {{ $page->balanceFormatted }}</p>
            ...
            <div class="mt-3 grid grid-cols-2 gap-3">
                <div class="rounded-xl border border-slate-100 bg-slate-50/80 p-3">
                    ...
                    <p class="mt-1 flex items-center gap-1 text-lg font-bold text-slate-900">
                        {{ $page->avgRating !== null ? number_format($page->avgRating, 1, ',', '') : '—' }}
                ...
                <a href="{{ route('muthowif.bookings.index') }}" class="rounded-xl border border-slate-100 bg-slate-50/80 p-3 transition hover:border-amber-200">
                    ...
                    <p class="mt-1 text-lg font-bold text-slate-900">{{ $page->pendingBookingsCount }}</p>
                </a>
            </div>
        </div>
        ...
        <div class="order-3 lg:order-none">
            @include('partials.dashboard-muthowif-share', ['mp' => $page->profile, 'profileUrl' => route('layanan.show', $page->profile), 'userInitial' => $page->userInitial])
        </div>
        ...
        <div class="order-9 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm lg:order-none">
            ...
            <div class="mt-3 flex items-center gap-3">
                <div class="relative h-12 w-12 shrink-0">
                    <img src="{{ $page->photoUrl }}" alt="" class="h-full w-full rounded-full object-cover ring-2 ring-slate-100" onerror="this.classList.add('hidden'); this.nextElementSibling.classList.remove('hidden');" />
                    <span class="hidden flex h-full w-full items-center justify-center rounded-full bg-baytgo text-lg font-bold text-white">{{ $page->userInitial }}</span>
                </div>
                <div class="min-w-0">
                    <p class="truncate font-bold text-slate-900">{{ $page->userName }}</p>
                    <p class="truncate text-xs text-slate-500">{{ $page->userEmail }}</p>
                </div>
            </div>
                        ...
                        <li class="flex gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if ($activity['time_label'])
                                    <p class="mt-0.5 text-xs text-slate-400">{{ $activity['time_label'] }}</p>
                                @endif
                            </div>
                        </li>
                    @endforeach
