    ...
    <div class="grid lg:grid-cols-[minmax(0,340px)_1fr]">
        <div class="relative aspect-[4/5] min-h-[280px] bg-slate-100 sm:min-h-[320px] lg:aspect-auto lg:min-h-[420px]">
            <img
                ...
                alt="{{ $page->muthowifName }}"
                class="absolute inset-0 h-full w-full object-cover"
                loading="eager"
                onerror="this.onerror=null; this.src={!! json_encode($page->fallbackSvg) !!}"
            >
        ...
        <div class="ui-card-pad-lg flex flex-col justify-center lg:p-10">
            ...
            <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl lg:text-4xl">{{ $page->muthowifName }}</h1>

            @if (filled($page->workLocation))
            ...
            @if ($page->reviewsCount === 0 && $page->confirmedBookings === 0)
            ...
            @if ($page->languageList !== [])
                <div class="mt-4 flex flex-wrap items-center gap-3">
                    @foreach (array_slice($page->languageList, 0, 4) as $lang)
            ...
            <dl class="mt-6 grid grid-cols-2 gap-4 border-y border-slate-100 py-5 sm:grid-cols-4">
                <div>
                    ...
                    <dd class="mt-1 text-sm font-bold text-slate-900 line-clamp-2">{{ $page->experienceStat }}</dd>
                </div>
                <div>
                    <dt class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{{ __('marketplace.show.stat_pilgrims') }}</dt>
                    <dd class="mt-1 text-sm font-bold text-slate-900">{{ $page->pilgrimStat }}</dd>
                </div>
                <div>
                    <dt class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{{ __('marketplace.show.stat_languages') }}</dt>
                    <dd class="mt-1 text-sm font-bold text-slate-900">{{ $page->langStat }}</dd>
                    @if (count($page->languageList) > 0)
                        <dd class="mt-0.5 text-[11px] text-slate-500 line-clamp-1">{{ __('marketplace.show.stat_languages_names', ['names' => implode(', ', array_slice($page->languageList, 0, 3))]) }}</dd>
                    @endif
                </div>
                <div>
                    ...
                    <dd class="mt-1 flex items-center gap-1 text-sm font-bold text-slate-900">
                        @if ($page->reviewsCount > 0 && $page->avgRating !== null)
                            ...
                            {{ $page->avgRating }} <span class="font-normal text-slate-500">({{ $page->reviewsCount }} review)</span>
                        @else
            ...
            <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                @if ($page->canBook)
                    <a href="{{ $page->bookingPageUrl }}" class="ui-btn-primary flex-1 px-6 text-base sm:flex-none sm:min-w-[200px]">
                        {{ __('marketplace.show.book_now') }}
                    </a>
                @else
                    <a href="{{ $page->bookingPageUrl }}" class="ui-btn-secondary flex-1 px-6 text-base sm:flex-none sm:min-w-[200px]">
