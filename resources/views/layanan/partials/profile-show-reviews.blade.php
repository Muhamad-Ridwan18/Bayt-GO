        ...
        <div class="mt-6 flex flex-col gap-6 lg:flex-row lg:items-start">
            <div class="shrink-0 rounded-2xl bg-slate-50 px-8 py-6 text-center ring-1 ring-slate-100 lg:min-w-[140px]">
                ...
                <div class="mt-2 flex justify-center gap-0.5" aria-hidden="true">
                    @for ($s = 1; $s <= 5; $s++)
                        <svg class="h-4 w-4 {{ $s <= (int) round((float) $avgRating) ? 'text-gold' : 'text-slate-200' }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                    @endfor
                </div>
                <p class="mt-1 text-xs text-slate-500">({{ $reviewsCount }} review)</p>
            </div>
