...
<form method="GET" action="{{ route('layanan.index') }}" class="rounded-3xl border border-slate-200/90 bg-white p-5 sm:p-6 md:p-7 shadow-market ring-1 ring-brand-100/60 w-full min-w-0">
    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-end lg:gap-x-3 lg:gap-y-4">
        ...
        <div class="w-full sm:max-w-md lg:w-[13.5rem] lg:max-w-none lg:flex-shrink-0">
            ...
            <input type="date" name="end_date" id="end_date" value="{{ $endDate }}"
                   ...
                   class="{{ $dateClass }} mt-2"
                   title="Kosongkan jika hanya satu hari — dianggap sama dengan tanggal mulai" />
        </div>
        <div class="w-full lg:min-w-[12rem] lg:flex-1">
            <label for="q" class="block text-sm font-medium text-slate-700">Nama muthowif</label>
    ...
    <p class="mt-4 text-xs leading-relaxed text-slate-500">
        <span class="text-slate-600 font-medium">Cepat:</span>
        tanggal selesai boleh dikosongkan jika hanya satu hari (dianggap sama dengan mulai).
        Nama opsional.
        <span class="hidden sm:inline"> </span>
        <span class="block sm:inline mt-1 sm:mt-0 text-slate-400">Hasil hanya muthowif yang tidak libur dan slot-nya kosong pada rentang tersebut.</span>
    </p>
</form>
