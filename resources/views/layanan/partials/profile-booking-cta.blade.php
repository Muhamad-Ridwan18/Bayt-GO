...
<x-ui.card pad="md" class="relative overflow-hidden" aria-label="{{ __('layanan.profile_cta.section_aria') }}">
    <div class="relative">
        ...
        @if ($page->canBook)
            <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                @if ($page->searchRangeLabel)
                    <span class="inline-flex items-center gap-2 rounded-xl bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-900 ring-1 ring-brand-200/80">{{ $page->searchRangeLabel }}</span>
                @endif
            </div>
            <p class="mt-3 text-xs leading-relaxed text-slate-500">{{ __('layanan.profile_cta.hint_wide_form') }}</p>
            <a href="{{ $page->bookingPageUrl }}" class="mt-4 inline-flex text-sm font-semibold text-brand-700 underline decoration-brand-600/35 underline-offset-2 hover:text-brand-800">
                {{ __('layanan.profile_cta.go_booking') }} →
            </a>
        @elseif ($reason === 'guest')
            <x-ui.alert type="info" class="mt-5">
                <p>{{ __('marketplace.panel.guest_intro') }}</p>
                <a href="{{ route('login.intended', ['next' => $page->bookingPageUrl]) }}" class="ui-btn-primary mt-4 w-full sm:w-auto">{{ __('marketplace.panel.guest_login') }}</a>
        ...
        @elseif ($reason === 'invalid_dates')
            ...
            <a href="{{ route('layanan.index', ['start_date' => $page->startDate, 'end_date' => $page->endDate]) }}" class="mt-3 inline-flex text-sm font-semibold text-brand-700 underline decoration-brand-600/35 underline-offset-2 hover:text-brand-800">{{ __('layanan.booking_search_again') }}</a>
        ...
        @elseif ($reason === 'jadwal_tidak_tersedia')
            <x-ui.alert type="warning" class="mt-5">
                {!! __('marketplace.panel.jadwal_tidak_tersedia_html', [
                    'range' => $page->searchRangeLabel ?? '—',
                    'link' => '<a href="'.e(route('layanan.index', array_filter(['start_date' => $page->startDate, 'end_date' => $page->endDate !== '' ? $page->endDate : null]))).'" class="font-semibold underline">'.e(__('layanan.booking_panel_link')).'</a>',
                ]) !!}
            </x-ui.alert>
        ...
        @elseif ($reason !== '')
            ...
            <a href="{{ $page->bookingPageUrl }}" class="mt-3 inline-flex text-sm font-semibold text-brand-700 underline decoration-brand-700/35 underline-offset-2 hover:text-brand-800">{{ __('layanan.profile_cta.open_booking_page') }}</a>
        @endif
    </div>
</x-ui.card>
