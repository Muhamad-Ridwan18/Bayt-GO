...
<x-marketplace-layout title="Cari muthowif">
    <div class="space-y-10">
        ...
        <div class="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-gradient-to-br from-white via-brand-50/40 to-amber-50/30 px-6 py-8 sm:px-10 sm:py-10 shadow-market ring-1 ring-slate-100/80">
            ...
            <div class="relative max-w-3xl">
                <p class="text-xs font-bold uppercase tracking-wider text-brand-700">Marketplace pendamping</p>
                <h1 class="mt-2 text-3xl sm:text-4xl font-bold tracking-tight text-slate-900">Cari muthowif yang cocok untuk perjalanan Anda</h1>
                <p class="mt-3 text-base text-slate-600 leading-relaxed">
                    Pilih tanggal — kami hanya menampilkan muthowif yang <strong>benar-benar tersedia</strong> (bukan hari libur, tidak bentrok booking). Profil terverifikasi, harga transparan.
                </p>
        ...
        @if (! $hasDateSearch)
            <div class="rounded-2xl border border-dashed border-brand-300/80 bg-gradient-to-br from-brand-50/60 to-white p-10 text-center shadow-inner">
                <p class="font-semibold text-slate-900">Pilih tanggal mulai (dan selesai jika lebih dari satu hari), lalu tekan <strong class="text-brand-800">Cari</strong>.</p>
                <p class="mt-2 text-sm text-slate-600 max-w-md mx-auto">Daftar muthowif hanya muncul setelah tanggal diisi — supaya Anda tidak membuang waktu pada yang tidak available.</p>
            </div>
        @else
            @if ($dateErrors === null || $dateErrors->isEmpty())
                @if (filled($rangeLabel))
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                        <p class="text-sm text-slate-600">
                            Rentang <span class="font-semibold text-slate-900">{{ $rangeLabel }}</span>
                            @if ($profiles->total() > 0)
                                <span class="text-slate-400">·</span>
                                <span class="font-semibold text-brand-800">{{ $profiles->total() }} pendamping siap</span>
                            @endif
                        </p>
                    </div>
                @endif
                ...
                @if ($profiles->isEmpty())
                    <div class="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
                        <p class="text-slate-800 font-medium">Belum ada muthowif yang cocok pada rentang ini</p>
                        <p class="mt-2 text-sm text-slate-600 max-w-md mx-auto">Semua sedang libur atau slot sudah terisi. Coba ubah tanggal atau kata kunci nama.</p>
                    </div>
                @else
