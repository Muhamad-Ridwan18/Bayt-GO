<x-app-layout>
    <div @class([
        ...
        'bg-gradient-to-b from-welcomeCanvas via-white to-slate-50/80' => $dashboardKind === 'customer',
        'overflow-x-hidden bg-slate-50' => in_array($dashboardKind, ['admin', 'muthowif'], true),
        'overflow-hidden bg-gradient-to-b from-slate-100 via-slate-50 to-white' => ! in_array($dashboardKind, ['customer', 'admin', 'muthowif'], true),
        '!pt-0 !pb-0' => $dashboardKind === 'muthowif',
    ])>
        @unless (in_array($dashboardKind, ['customer', 'admin', 'muthowif'], true))
        ...
        @if ($dashboardKind === 'customer')
            <x-dashboard.customer.page :page="$customerPage" />
        @else
            <x-page-container class="ui-stack-tight relative">
                @unless ($dashboardKind === 'admin')
                    <x-campaign-carousel :campaigns="$activeCampaigns ?? collect()" />
                @endunless

                @if ($dashboardKind === 'muthowif')
                    <x-dashboard.muthowif.page :page="$muthowifPage" />
                @elseif ($dashboardKind === 'admin')
                    @include('partials.dashboard-admin', ['page' => $adminPage])
                @elseif ($dashboardKind === 'muthowif_pending')
                ...
                @unless (in_array($dashboardKind, ['admin', 'muthowif'], true))
