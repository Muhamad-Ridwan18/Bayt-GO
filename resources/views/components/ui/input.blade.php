...
@php
    ...
    $baseClass = 'block w-full rounded-xl border-slate-200 bg-slate-50/50 py-3 shadow-sm transition focus:border-baytgo focus:bg-white focus:ring-baytgo/20';
    $withIconClass = $iconPath ? 'ps-11' : 'px-4';
@endphp
...
<div class="relative">
    ...
    @if ($type === 'password')
        <x-password-input {{ $attributes->class([$baseClass, $withIconClass, $iconPath ? 'pe-11' : '']) }} />
    @else
        <x-text-input type="{{ $type }}" {{ $attributes->class([$baseClass, $withIconClass]) }} />
    @endif
</div>
