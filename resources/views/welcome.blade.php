...
@endphp
    ...
    <body class="font-welcome antialiased text-slate-800 bg-white min-h-screen selection:bg-gold-light selection:text-baytgo-950">
        <div class="min-h-screen flex flex-col">
            {{-- Top navigation (light, design reference) --}}
            <header class="sticky top-0 z-40 border-b border-slate-100 bg-white">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative flex min-h-[4.25rem] items-center justify-between gap-4 lg:gap-6">
                    <a href="{{ url('/') }}" class="relative z-10 flex shrink-0 items-center gap-2.5 group">
                        <x-site-logo variant="welcome" class="rounded-xl ring-1 ring-slate-200/70" />
                        <span class="text-lg font-bold tracking-tight text-baytgo">Bayt<span class="text-gold-muted">Go</span></span>
                    </a>

                    <nav class="hidden lg:flex absolute left-1/2 top-1/2 max-w-none -translate-x-1/2 -translate-y-1/2 items-center gap-0.5 text-sm font-semibold">
                    ...
                    <div class="relative z-10 flex shrink-0 items-center gap-2 sm:gap-3">
                        <x-language-switcher variant="welcome-menu" class="hidden sm:block" />
                        @if ($contactLink)
                            <a href="{{ $contactLink }}" target="_blank" rel="noopener noreferrer" class="hidden sm:inline-flex items-center rounded-xl bg-baytgo px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-baytgo/20 transition hover:bg-baytgo-800">
                                {{ __('nav.contact_us') }}
                            </a>
                        @endif
                        @auth
                            <a href="{{ route('dashboard') }}" class="hidden sm:inline-flex rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-baytgo hover:text-baytgo">{{ __('nav.home') }}</a>
                        @else
                            @if (Route::has('login'))
                                <a href="{{ route('login') }}" class="hidden sm:inline-flex rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 transition hover:text-baytgo">{{ __('layanan.guest_header_login') }}</a>
                            @endif
                        @endauth
                    </div>
                </div>
                <div class="flex items-center justify-center border-t border-slate-100 px-4 py-2.5 sm:hidden">
