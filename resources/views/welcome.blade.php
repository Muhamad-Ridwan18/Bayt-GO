...
@endphp
    <head>
        ...
        <title>{{ config('app.name', 'BaytGo') }} — Marketplace umrah</title>
            ...
            <header class="sticky top-0 z-20 border-b border-white/10 bg-slate-900/95 backdrop-blur-md text-white">
                <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
                    ...
                    <nav class="flex items-center gap-1 sm:gap-2 text-sm flex-wrap justify-end">
                        ...
                        @else
                            @if (Route::has('login'))
                                <a href="{{ route('login') }}" class="px-3 sm:px-4 py-2 rounded-xl font-medium text-white/90 hover:bg-white/10 transition">Masuk</a>
                            @endif
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="px-4 py-2 rounded-xl font-semibold bg-brand-500 text-white shadow-md hover:bg-brand-400 transition ring-1 ring-white/20">Daftar</a>
                            @endif
                        @endauth
                    </nav>
                </div>
            ...
            <main class="flex-1">
                ...
                <section class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-brand-900 to-amber-950 text-white">
                    ...
                    <div class="relative max-w-6xl mx-auto px-4 sm:px-6 pt-12 sm:pt-16 pb-12 sm:pb-16">
                        ...
                        <div class="mt-10 max-w-4xl">
                            <p class="text-sm font-medium text-brand-100/90 mb-3">Cari ketersediaan</p>
                        ...
                        <div class="mt-10 flex flex-wrap gap-3 sm:gap-4 text-sm">
                            @foreach ([
                                ['label' => 'Muthowif terverifikasi', 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                                ['label' => 'Filter tanggal & slot booking', 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5a2.25 2.25 0 002.25-2.25m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5a2.25 2.25 0 012.25 2.25v7.5'],
                                ['label' => 'Group &amp; private + add-on', 'icon' => 'M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z'],
                            ] as $badge)
                ...
                <section class="max-w-6xl mx-auto px-4 sm:px-6 -mt-6 relative z-10">
                    <div class="rounded-2xl bg-white shadow-market border border-slate-200/80 p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <p class="font-semibold text-slate-900">Belum punya akun?</p>
                            <p class="mt-1 text-sm text-slate-600">Daftar sebagai jamaah atau muthowif — gratis mulai.</p>
                        </div>
                ...
                <section class="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20">
                    <div class="max-w-2xl mb-10">
                        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Cara pakai marketplace</h2>
                        <p class="mt-2 text-slate-600">Empat langkah singkat dari pencarian sampai transaksi.</p>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        @foreach ([
                            ['n' => '1', 'title' => 'Tanggal & nama', 'desc' => 'Cari rentang perjalanan dan opsional filter nama muthowif.'],
                            ['n' => '2', 'title' => 'Lihat profil', 'desc' => 'Layanan group/private, harga harian, add-on, dan jadwal libur.'],
                            ['n' => '3', 'title' => 'Booking & bayar', 'desc' => 'Slot yang bentrok otomatis disembunyikan dari pencarian.'],
                            ['n' => '4', 'title' => 'Berangkat', 'desc' => 'Pendamping siap di Tanah Suci sesuai kesepakatan.'],
                        ] as $step)
